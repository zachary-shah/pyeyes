...
## Overview of Tasks
The next steps for this repo are as follows:
...
- [ ] Translate current test scripts to pytests
- [ ] Ensure different data input modes are supported. this includes
    - [ ] Dict of numpy inputs for multiple datasets
    - [ ] Single array for viewing only one dataset
- [ ] Ensure different named dimensions input types are supported.
    - this is currenlty implemented in test_dim_input.py, but need to translate to pytests still
- [ ] Add scripted ways to test GUI features through pytest
- [ ] To start, let's test just one feature modification, like editing a value of a sdim widget.
- [ ] Then we will try more complex feature changes
