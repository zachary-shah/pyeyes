...
from ..utils import RMSE, normalize, tonp
...
def plot_cplx_recons_gif(
    recons: Sequence[np.ndarray],
    ...
    print("Saving gif of recons evolution...")
    ...
    for i in range(N_method):
        ...
        im_phase = axs[1, i].imshow(
            np.angle(recons[i][0]), cmap="jet", vmin=-np.pi, vmax=np.pi
        )
...
class PlotReconErrorDynamic:
    ...
    def __init__(
        ...
        assert mode in [
            "mag_phase",
            "real_imag",
        ], "Mode must be 'mag_phase' or 'real_imag'"
        ...
        if plot_trim is not None:
            X, Y = imgs.shape[1:3]
            self.pslc = slice(plot_trim[0], X - plot_trim[0]), slice(
                plot_trim[1], Y - plot_trim[1]
            )
        ...
        if mode == "real_imag":
            imgs = np.real(imgs) * np.exp(
                1j * np.imag(imgs)
            )  # hacky way to convert to complex
        ...
        for i in range(self.Nm + 1):
            self.axim[0, i] = ax[0, i].imshow(
                np.zeros(self.im_shape),
                vmin=vmin,
                vmax=vmax,
                cmap=cmap,
                extent=[0, self.im_shape[1], 0, self.im_shape[0]],
            )
            self.axim[1, i] = ax[1, i].imshow(
                np.zeros(self.im_shape),
                vmin=vmin,
                vmax=vmax * error_scale,
                cmap=cmap,
                extent=[0, self.im_shape[1], 0, self.im_shape[0]],
            )
            self.axim[2, i] = ax[2, i].imshow(
                np.zeros(self.im_shape),
                vmin=-np.pi * phase_scale,
                vmax=np.pi * phase_scale,
                cmap="jet",
                extent=[0, self.im_shape[1], 0, self.im_shape[0]],
            )
            ...
            if self.show_nrmse and i >= 1:
                self.t[i - 1] = self.ax[0, i].text(
                    self.im_shape[1] // 2,
                    self.im_shape[0] - 5,
                    f"NRMSE={self.nrmses[i-1,self.z]:0.4f}",
        ...
        self.ax[0, 0].set_ylabel(
            "Magnitude", color=self.secondary_color, fontsize=font_size
        )
        self.ax[1, 0].set_ylabel(
            f"Difference ({1/self.error_scale:.1f}x)",
            color=self.secondary_color,
            fontsize=font_size,
        )
        self.ax[2, 0].set_ylabel(
            "Phase", color=self.secondary_color, fontsize=font_size
        )
        self.ax[0, 0].set_title(
            self.ref_title, color=self.secondary_color, fontsize=font_size
        )
        for i in range(self.Nm):
            self.ax[0, i + 1].set_title(
                self.titles[i], color=self.secondary_color, fontsize=font_size
            )

        self.suptit = self.fig.suptitle(
            f"Slice {self.z}: {self.suptitle}",
            color=self.secondary_color,
    ...
    def update_image(self):
        ...
        for i in range(self.Nm):
            ...
            for j in range(3):
                self.axim[j, i + 1].set_extent(
                    [0, self.im_shape[1], 0, self.im_shape[0]]
                )

        return


def plot_recon_errors_color(
    imgs,
    ref_img,
    titles,
    ref_title="Reference",
    ...
    if np.iscomplexobj(imgs[0]):
        print(
            "Warning: Input color images are complex-valued. Taking magnitude of images."
        )
...
class PlotReconErrorColorDynamic:
    ...
    def __init__(
        ...
        if plot_trim is not None:
            X, Y = imgs.shape[1:3]
            self.pslc = (
                slice(plot_trim[0], X - plot_trim[0]),
                slice(plot_trim[1], Y - plot_trim[1]),
                slice(None),
            )
            self.im_shape = (X - 2 * plot_trim[0], Y - 2 * plot_trim[1])
        else:
        ...
        nrmses = np.zeros((self.Nm, self.Nz))
        diffs = np.zeros(
            (self.Nm, *self.full_im_shape, self.Nz, self.C), dtype=np.float32
        )
        targs = np.zeros(
            (self.Nm, *self.full_im_shape, self.Nz, self.C), dtype=np.float32
        )
        ...
        for i in range(self.Nm):
            for z in range(self.Nz):
                if norm_scale:
                    x = np.abs(
                        normalize(
                            imgs[i, ..., z, :], ref_img[..., z, :], ofs=True, mag=True
                        )
                    )
                else:
                    x = imgs[i, ..., z, :]
        ...
        fig, ax = plt.subplots(
            2, self.Nm + 1, figsize=(H * (self.Nm + 1), W * 2.06), dpi=dpi
        )
        ...
        for i in range(self.Nm + 1):
            self.axim[0, i] = ax[0, i].imshow(
                np.zeros((*self.im_shape, 3), dtype=np.float32),
                extent=[0, self.im_shape[1], 0, self.im_shape[0]],
            )
            self.axim[1, i] = ax[1, i].imshow(
                np.zeros((*self.im_shape, 3), dtype=np.float32),
