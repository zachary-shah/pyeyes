...
class PlotDTIDynamic:
    ...
    def __init__(
        ...
        if self.plot_trim is not None:
            X, Y = FA.shape[:2]
            self.px, self.py = slice(self.plot_trim[0], X - self.plot_trim[0]), slice(self.plot_trim[1], Y - self.plot_trim[1])
        ...
        self.axim[0,3] = ax[0,3].imshow(FA[self.px, self.py, self.z], cmap='gray', vmin=0, vmax=1)
        cbars.append(fig.colorbar(self.axim[0,3], cax=inset_axes(ax[0,4], **cbar_ax_params), format='%d', ticks=[0, 1], **cbar_pm))

        self.axim[0,4] = ax[0,5].imshow(MO[self.px, self.py, self.z], cmap='coolwarm', vmin=-1, vmax=1)
        cbars.append(fig.colorbar(self.axim[0,4], cax=inset_axes(ax[0,6], **cbar_ax_params), format='%d', ticks=[-1, 0, 1], **cbar_pm))

        self.axim[1,0] = ax[1,0].imshow(L1[self.px, self.py, self.z], cmap='gray', vmin=0, vmax=L_MAX)
        self.axim[1,1] = ax[1,1].imshow(L2[self.px, self.py, self.z], cmap='gray', vmin=0, vmax=L_MAX)
        self.axim[1,2] = ax[1,2].imshow(L3[self.px, self.py, self.z], cmap='gray', vmin=0, vmax=L_MAX)
        self.axim[1,3] = ax[1,3].imshow(MD[self.px, self.py, self.z], cmap='gray', vmin=0, vmax=L_MAX)
        cbars.append(fig.colorbar(self.axim[1,3], cax=inset_axes(ax[1,4], **cbar_ax_params), format='%0.4f', **cbar_pm))
        
        self.axim[1,4] = ax[1,5].imshow(S0[self.px, self.py, self.z], cmap='gray', vmin=0, vmax=S_MAX)
        cbars.append(fig.colorbar(self.axim[1,4], cax=inset_axes(ax[1,6], **cbar_ax_params), format='%d', **cbar_pm))
        ...
        self.fig.suptitle(f"Slice {self.z}: {self.suptitle}", color=self.secondary_color)
